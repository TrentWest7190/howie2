...
#pragma shader_feature VHS_JITTER_V_ON
	...
	float3 yiqDist(float2 uv, float m, float t) {
		...
		signal.x = rgb2yiq(tex2D(_InputTexture , float2(offsetX.r, uv.y)).rgb).x;
		signal.y = rgb2yiq(tex2D(_InputTexture , float2(offsetX.g, uv.y)).rgb).y;
		signal.z = rgb2yiq(tex2D(_InputTexture , float2(offsetX.b, uv.y)).rgb).z;
		return signal;
	}
#endif
#if VHS_TWITCH_V_ON	
