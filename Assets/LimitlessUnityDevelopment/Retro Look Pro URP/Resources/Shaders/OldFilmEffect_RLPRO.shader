Shader "Hidden/Shader/OldFilmEffect_RLPRO"
{
		...
		float4 Frag0(Varyings i) : SV_Target
	{
		...
		float3 image = tex2D(_InputTexture, float2(suv.x, suv.y) ).rgb;
		...
		float4 result = float4(oldImage * vI, tex2D(_InputTexture, float2(suv.x, suv.y)).a);
		result = lerp(result, tex2D(_InputTexture, i.uv), 1 - Fade);
		return result;
	}
