Shader "Hidden/Shader/NTSCEncode_RLPRO"
{
	...
	float4 CompositeSample(float2 UV) 
	{
		...
		float3 Texel0 = tex2D(_InputTexture, C0 ).rgb;
		float3 Texel1 = tex2D(_InputTexture, C1 ).rgb;
		float3 Texel2 = tex2D(_InputTexture, C2 ).rgb;
		float3 Texel3 = tex2D(_InputTexture, C3 ).rgb;
