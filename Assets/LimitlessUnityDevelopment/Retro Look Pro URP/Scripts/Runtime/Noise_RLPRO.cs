...
public class Noise_RLPRO : ScriptableRendererFeature
{
	...
	public class Noise_RLPROPass : ScriptableRenderPass
	{
		...
		void Render(CommandBuffer cmd, ref RenderingData renderingData)
		{
			...
			if (retroEffect.mask.value)
			{
				RetroEffectMaterial.SetFloat(alphaTexV, 1);
				if (retroEffect.maskTexture.value != null)
					RetroEffectMaterial.SetTexture(_AlphaMapTexV, retroEffect.maskTexture.value);
			}
			else
			{
				RetroEffectMaterial.SetFloat(alphaTexV, 0);
			}
