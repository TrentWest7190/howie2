...
namespace Nexweron.WebCamPlayer
{
	...
	public class WebCamStreamEditor : Editor
	{
		...
		SerializedProperty _requestedDeviceNameProperty;
		SerializedProperty _requestedDeviceIndexProperty;
		
		SerializedProperty _resolutionModeProperty;
		SerializedProperty _requestedSizeProperty;
		...
		void OnEnable() {
			...
			_requestedDeviceNameProperty = serializedObject.FindProperty("_requestedDeviceName");
			_requestedDeviceIndexProperty = serializedObject.FindProperty("_requestedDeviceIndex");
				
			_resolutionModeProperty = serializedObject.FindProperty("_resolutionMode");
			_requestedSizeProperty = serializedObject.FindProperty("_requestedSize");
		...
		public override void OnInspectorGUI() {
			...
			var targetInstance = serializedObject.targetObject as WebCamStream;
			
			//Default
			DrawPropertiesExcluding(serializedObject, new [] {
				"m_Script",
				_deviceModeProperty.name, 
				_requestedDeviceNameProperty.name,
				_requestedDeviceIndexProperty.name,
				_resolutionModeProperty.name,
				_requestedSizeProperty.name,
				_fpsModeProperty.name,
				_requestedFPSProperty.name,
				_playOnAwakeProperty.name });
			
			EditorGUI.BeginDisabledGroup(EditorApplication.isPlaying && (targetInstance.webCamTexture != null || targetInstance.isAuthorization));
			{
				//Device
				...
				++EditorGUI.indentLevel;
				{
					if (EditorGUILayout.BeginFadeGroup(_showDeviceNameProperty.faded)) {
						EditorGUILayout.PropertyField(_requestedDeviceNameProperty);
					}
					EditorGUILayout.EndFadeGroup();

					if (EditorGUILayout.BeginFadeGroup(_showDeviceIndexProperty.faded)) {
						EditorGUILayout.PropertyField(_requestedDeviceIndexProperty);
					}
					EditorGUILayout.EndFadeGroup();
				}
				--EditorGUI.indentLevel;
				...
				++EditorGUI.indentLevel;
				{
					if (EditorGUILayout.BeginFadeGroup(_showResolutionProperties.faded)) {
						EditorGUILayout.PropertyField(_requestedSizeProperty);
					}
					EditorGUILayout.EndFadeGroup();
				}
				--EditorGUI.indentLevel;
				...
				//Fps
				EditorGUI.BeginDisabledGroup(resolutionMode == WebCamStream.ResolutionMode.Auto);
				{
					EditorGUILayout.PropertyField(_fpsModeProperty);
					if (resolutionMode == WebCamStream.ResolutionMode.Auto) {
			...
			//Buttons
			bool isEnablePlayButton = EditorApplication.isPlaying && (targetInstance.webCamTexture == null || (targetInstance.webCamTexture != null && !targetInstance.webCamTexture.isPlaying));
			bool isEnablePauseButton = EditorApplication.isPlaying && (targetInstance.webCamTexture != null && targetInstance.webCamTexture.isPlaying);
			bool isEnableStopButton = EditorApplication.isPlaying && targetInstance.webCamTexture != null;
			...
			if (targetInstance.isPrepared) {
				var webCamTexture = targetInstance.webCamTexture;
				deviceNameValue = webCamTexture.deviceName;
				deviceIndexValue = "["+ArrayUtility.IndexOf(WebCamTexture.devices, targetInstance.webCamDevice)+"]";
